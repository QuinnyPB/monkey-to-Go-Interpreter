...
func TestDefineMacro(t *testing.T) {
	...
	if len(macro.Parameters) != 2 {
		t.Fatalf("mymacro.Parameters != 2, got=%d", len(macro.Parameters))
	}

	if macro.Parameters[0].String() != "x" {
		t.Fatalf("mymacro.Parameters[0] != x, got=%q", macro.Parameters[0])
	}

	if macro.Parameters[1].String() != "y" {
		t.Fatalf("mymacro.Parameters[1] != y, got=%q", macro.Parameters[1])
	}
